...
public class FNBlockVisitor extends SpecSimplifierClassBaseVisitor {
	...
	public void visitJmlFieldDeclaration(JmlFieldDeclaration self) {
		String className = FieldRenameUtil.extractClassNameForFieldRenameSupport(self.getField());
		String new_name;
		...
		JmlFieldDeclarationExtension newSelf = new JmlFieldDeclarationExtension(self, new_name);
	...
	public void visitJmlAssignableClause(JmlAssignableClause self) {
		...
		for (int i = 0; i < self.storeRefs().length; i++) {
			...
			if (self.storeRefs()[i] instanceof JmlStoreRefExpression) {
				...
				String  storeRefType;
				if (jmlStoreRefExpression.expression().toString().contains(" ")){
					storeRefType = (jmlStoreRefExpression.expression().toString().split("\\s")[0]).replace('.','_');
				} else {
					storeRefType = this.currentClassName;
				}
