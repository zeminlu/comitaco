...
public class StrykerASTVisitor extends ASTVisitor {
	...
	private static final String MUT_ID_COMMENT_PREFIX = "//mutID ";
	...
	public String getLineComment(int commentIndex) {
		...
		// Tiene comentario el statement, potencialmente sea el de mutacion que
		// necesito
		String lineComment = "";
		boolean flag = true;
		while (flag) {
			lineCommentNode = ((LineComment) unit.getCommentList().get(
					commentIndex));
			lineComment = source.substring(
					lineCommentNode.getStartPosition(),
					lineCommentNode.getStartPosition()
							+ lineCommentNode.getLength());
	...
	public boolean preVisit2(ASTNode node) {
		if (node instanceof Statement && !(node instanceof Block)
			...
			// Ojo con los replace, no se si al reemplazar un padre, y luego
			// reemplazar el hijo, si el hijo sera reemplazado o no,
			// porque en el rewrite ya se cambio ese hijo por una copia,
			// entonces quiz��z no lo encuentra.
			...
			if (thenStatement instanceof Block) {
				Statement thenFirstStatement = (Statement) ((Block) thenStatement)
						.statements().get(0);
				if (!(thenFirstStatement instanceof IfStatement)
					...
					if (commentIndex >= 0) {
						...
						ASTNode thenNodes[] = {
								getAppendToFileExpressionStatement("if(!("
										+ ifExpression.toString()
										+ ")){throw new RuntimeException();}"
										+ '\n'),
								// getAppendToFileExpressionStatement("assert("
								// + ifExpression.toString() + ");" + '\n'),
					...
					rewrite.getListRewrite(thenStatement,
			...
			if (elseStatement != null) {
				...
				if (elseStatement instanceof Block) {
					Statement elseFirstStatement = (Statement) ((Block) elseStatement)
							.statements().get(0);
					...
					if (!(elseFirstStatement instanceof IfStatement)
						...
						newElseNode = ASTNode.copySubtree(ast,
								elseFirstStatement);
						customNodes.add(elseFirstStatement);
						int commentIndex = unit
								.lastTrailingCommentIndex(elseFirstStatement);
						if (commentIndex >= 0) {
							ASTNode nodes[] = {
									getAppendToFileExpressionStatement("if("
											+ ifExpression.toString()
						...
						rewrite.getListRewrite(elseStatement,
								Block.STATEMENTS_PROPERTY).insertFirst(
								getAppendToFileExpressionStatement("if("
										+ ifExpression.toString()
										+ "){throw new RuntimeException();}"
					...
					int commentIndex = unit
							.lastTrailingCommentIndex(elseStatement);
					if (commentIndex >= 0) {
						ASTNode nodes[] = {
								getAppendToFileExpressionStatement("if("
										+ ifExpression.toString()
			...
			Expression whileExpression = ((WhileStatement) node)
					.getExpression();
			...
			if (whileBody instanceof Block) {
				Statement whileBodyFirstStatement = (Statement) ((Block) whileBody)
						.statements().get(0);
				if (!(whileBodyFirstStatement instanceof IfStatement)
					...
					newWhileBodyFirstStatementNode = ASTNode.copySubtree(ast,
							whileBodyFirstStatement);
					customNodes.add(whileBodyFirstStatement);
					int commentIndex = unit
							.lastTrailingCommentIndex(whileBodyFirstStatement);
					if (commentIndex >= 0) {
						ASTNode nodes[] = {
								getAppendToFileExpressionStatement("if(!("
					...
					rewrite.getListRewrite(whileBody, Block.STATEMENTS_PROPERTY)
							.insertFirst(
									getAppendToFileExpressionStatement("if(!("
											+ whileExpression.toString()
											+ ")){throw new RuntimeException();}"
				...
				if (commentIndex >= 0) {
					...
					rewrite.replace(whileBody,
							rewrite.createGroupNode(whileBodyNodes), null);
				}
			} else {
				newWhileBodyFirstStatementNode = ASTNode.copySubtree(ast,
						whileBody);
			...
			ASTNode assertNode = getAppendToFileExpressionStatement("if("
					+ whileExpression.toString()
					+ "){throw new RuntimeException();}" + '\n');
			// ASTNode assertNode =
			// getAppendToFileExpressionStatement("assert(!(" +
			// whileExpression.toString() + "));" + '\n');
			rewrite.getListRewrite(node.getParent(),
			...
			if (forBody instanceof Block) {
				Statement forBodyFirstStatement = (Statement) ((Block) forBody)
						.statements().get(0);
				if (!(forBodyFirstStatement instanceof IfStatement)
					...
					newForBodyFirstStatementNode = ASTNode.copySubtree(ast,
							forBodyFirstStatement);
					customNodes.add(forBodyFirstStatement);
					int commentIndex = unit
							.lastTrailingCommentIndex(forBodyFirstStatement);
					if (commentIndex >= 0) {
						ASTNode nodes[] = {
								getAppendToFileExpressionStatement("if(!("
					...
					rewrite.getListRewrite(forBody, Block.STATEMENTS_PROPERTY)
							.insertFirst(
									getAppendToFileExpressionStatement("if(!("
											+ forExpression.toString()
											+ ")){throw new RuntimeException();}"
				...
				newForBodyFirstStatementNode = ASTNode
						.copySubtree(ast, forBody);
				...
				// forExpression.toString() + ");" + '\n'),
				// newForBodyFirstStatementNode};
				rewrite.replace(forBody, rewrite.createGroupNode(forBodyNodes),
						null);
				// return true;
			}
			...
			Expression forExpression = ((EnhancedForStatement) node)
					.getExpression();
			...
			if (newNode.getName().getFullyQualifiedName()
					.contains(wrapper.getMethod())) {
				@SuppressWarnings("rawtypes")
				List statements = newNode.getBody().statements();

				Statement firstStatement = (Statement) statements.get(0);

				ASTNode newFirstStatement = ASTNode.copySubtree(ast,
						firstStatement);
				...
				// ASTNode nodes[] = {
				// es =
				// getWriteToFileExpressionStatement(newNode.toString().substring(0,
				// newNode.toString().indexOf('\n'))),
				// es3 =
				// getAppendToFileExpressionStatement(newFirstStatement.toString()),
				// newFirstStatement};
				ASTNode nodes[] = {
						es3 = getAppendToFileExpressionStatement(newFirstStatement
								.toString()), newFirstStatement };

				// customNodes.add(es);
				customNodes.add(es3);
				customNodes.add(firstStatement);

				rewrite.replace(firstStatement, rewrite.createGroupNode(nodes),
						null);

				Statement lastStatement = (Statement) statements.get(statements
						.size() - 1);

				ASTNode newLastStatement = ASTNode.copySubtree(ast,
						lastStatement);

				TryStatement es2, es4;

				// ASTNode lastNodes[] = {es4 =
				// getAppendToFileExpressionStatement(newLastStatement.toString()),
				// es2 = getAppendToFileExpressionStatement("}"),
				// newLastStatement};

				ASTNode lastNodes[] = {
						es4 = getAppendToFileExpressionStatement(newLastStatement
								.toString()), newLastStatement };
				...
				rewrite.replace(lastStatement,
						rewrite.createGroupNode(lastNodes), null);
				return true;
			} else {
	...
	public TryStatement getAppendToFileExpressionStatement(String value) {
		...
		SingleVariableDeclaration exception = ast
