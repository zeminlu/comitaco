	...
	private String declare_missing_int_literals(String alloy_str) {
		...
		for (int int_literal : int_literals) {
			if (int_literal >= 0) {
				...
				int neg_int_literal = -int_literal;

				if (!alloy_str.contains(String.format("JavaPrimitiveIntegerLiteralMinus%s extends JavaPrimitiveIntegerValue", neg_int_literal))) {
					buff.append("\n");
