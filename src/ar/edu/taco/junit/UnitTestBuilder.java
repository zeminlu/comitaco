...
public class UnitTestBuilder {
	...
	public void createUnitTest() throws IllegalArgumentException, IllegalAccessException, InstantiationException {
		...
		if (methodToCheckIsConstructor(clazz)) {
			Constructor<?> constructorToCheck = null;
			try {
				// Gets parameters types
				Class<?>[] parameterTypes = null;
				for (Constructor<?> aConstructor : clazz.getConstructors()) {
					String currentConstructorConsideredName = aConstructor.getName();
					if (aConstructor.getName().equals(recoveredInformation.getMethodToCheck())) {
						parameterTypes = aConstructor.getParameterTypes();
					}
				}

				constructorToCheck = clazz.getDeclaredConstructor(parameterTypes);
			...
			imports.add("java.lang.reflect.Constructor");
			...
			List<String> methodInvocationStatements = getConstructorInvocationStatements(clazz, constructorToCheck, paramsNames);
	...
	/**
	 * Generate the statements to invoke the constructor from a JUnit tests
	 * 
	 * @param clazz
	 * @param methodToCheck
	 * @return
	...
	private List<String> getConstructorInvocationStatements(Class<?> clazz, Constructor<?> constructorToCheck, List<String> paramsNames) {
		List<String> statements = new ArrayList<String>();
		...
		if (!constructorToCheck.isAccessible()) {
			statements.add("Constructor<?> method = getAccessibleConstructor(\"" + recoveredInformation.getClassToCheck() + "\", \""
					+ recoveredInformation.getMethodToCheck() + "\", true);");
			statements.add("try {");
			statements.add("    instance = (" + recoveredInformation.getClassToCheck() + ") method.newInstance(" + ((methodParameters.length() == 0) ? "null" : "new Object[]{" + methodParameters + "}") + ");");
