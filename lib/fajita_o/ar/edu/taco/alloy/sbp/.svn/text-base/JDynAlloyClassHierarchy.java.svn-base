...
public class JDynAlloyClassHierarchy {
	...
	private JDynAlloyClassHierarchy(List<JDynAlloyModule> src_jdynalloy_modules) {
		for (JDynAlloyModule jDynAlloyModule : src_jdynalloy_modules) {
			for (JRepresents represents : jDynAlloyModule.getRepresents()) {
				String rep[] = represents.getExpression().toString()
						.split("\\.");
				if (rep.length < 2) {
	...
	/**
	 * Used for matching the jDynAlloyModule fields with the actual fields
	 * defined in QF so as to consider only those fields in the intersection of
	 * both sets. Should be called before accessing any information from this
	 * object.
	 */
	...
	private void filterOutNonQFFields(Set<String> qfFieldNames,
			List<JField> fieldsToFilter) {
		Iterator<JField> it = fieldsToFilter.iterator();
		while (it.hasNext()) {
			JField field = it.next();
			if (!qfFieldNames.contains(field.getFieldVariable().getVariableId()
					.getString())) {
	...
	/**
	 * Gets all non-recursive and forward fields of type T' -> T where T is the
	 * given type classified by T'. The map will have entries of kind <T', {f1,
	 * ..., fn}> where fi: T' -> T.
	 */
	Map<String, Collection<JField>> getNonRecursiveAndForwardFieldsOfImageT(
			final String t) {
		...
		Collection<JField> nonRecursiveQFFieldsOfImageT = Collections2
				.transform(nonRecursiveFieldsOfImageT,
						new Function<JField, JField>() {
							@Override
							public JField apply(JField arg0) {
								return new JField(new AlloyVariable(SBPUtils
										.buildQFFieldName(arg0)), arg0
										.getFieldType());
							}
						});
		...
		Collection<JField> fRecursiveFieldsOfImageT = Collections2.transform(
				recursiveFieldsOfImageT, new Function<JField, JField>() {
					...
					public JField apply(JField field) {
						return new JField(new AlloyVariable("("
								+ SBPUtils.buildFFieldName(field) + ")"), field
								.getFieldType());
					}
				});
	...
	private Map<String, Collection<JField>> classifyByFrom(
			Collection<JField> fields) {
	...
	static JDynAlloyClassHierarchy buildFromModules(
			List<JDynAlloyModule> src_jdynalloy_modules) {
	...
	/**
	 * Checks whether the given field is recursive or not. A field is considered
	 * recursive if domain and codomain (minus the null value) match. For
	 * example, field next: LNode -> LNode+null is considered a recursive field.
	 */
	...
	private boolean isStaticField(JField field) {
		return SBPUtils.getOnlyFromOrThrowException(field)
				.equals("ClassFields");
	}
	...
	private boolean isArrayField(JField field) {
		return field.getFieldType().isTernaryRelation()
				|| field.getFieldVariable().getVariableId()
						.equals(new VariableId("Object_Array"));
	}
	...
	class JavaTypesHelper {
		...
		/**
		 * Used for matching the jDynAlloyModule fields with the actual fields
		 * defined in QF so as to consider only those fields in the intersection
		 * of both sets. Should be called before accessing any information from
		 * this object.
		 */
		private void matchWithQF(Set<String> qfFieldNames) {
			...
			javaTypes.remove(JavaPrimitiveIntegerValue.getInstance()
					.getModule().getSignature().getSignatureId());
			javaTypes.remove(JavaPrimitiveFloatValue.getInstance().getModule()
					.getSignature().getSignatureId());
			javaTypes.remove(JavaPrimitiveLongValue.getInstance().getModule()
					.getSignature().getSignatureId());
		...
		Collection<JField> nonRecursiveFieldsTargeting(final String javaType) {
			return Collections2.filter(nonRecursiveFields,
					new Predicate<JField>() {
						@Override
						public boolean apply(JField field) {
							return SBPUtils.getOnlyToOrThrowException(field)
									.equals(javaType);
						}
					});
		}
		...
		Collection<JField> recursiveFieldsTargeting(final String javaType) {
			return Collections2.filter(recursiveFields,
					new Predicate<JField>() {
						@Override
						public boolean apply(JField field) {
							return SBPUtils.getOnlyToOrThrowException(field)
		...
		/**
		 * @return The list of java types / exists f: T -> javaType given.
		 *         Ordered by T.
		 */
		Set<String> javaTypesWithFieldsTargeting(final String javaType) {
			...
			SortedSet<String> ret = Sets.newTreeSet(new Comparator<String>() {
				...
				public int compare(String o1, String o2) {
					return javaTypesAsList.indexOf(o1)
							- javaTypesAsList.indexOf(o2);
				}
			});
			Collection<JField> fieldsTargetingJavaType = Collections2.filter(
					fields, new Predicate<JField>() {
						@Override
						public boolean apply(JField field) {
							return SBPUtils.getOnlyToOrThrowException(field)
									.equals(javaType)
									&& javaTypes.contains(SBPUtils
											.getOnlyFromOrThrowException(field));
						}
					});
		...
		boolean existsField(final String fromJavaType, final String toJavaType) {
			for (JField field : fields) {
				if (SBPUtils.getOnlyFromOrThrowException(field).equals(
						fromJavaType)
						&& SBPUtils.getOnlyToOrThrowException(field).equals(
								toJavaType)) {
					return true;
				}
			}
	...
	class RootNodesHelper {
		...
		private Map<String, List<JField>> rootNodesOfJavaType = Maps
				.newHashMap();
		...
		private void processModule(JDynAlloyModule module) {
			for (JProgramDeclaration program : module.getPrograms()) {
				...
				if (!(sigId.equals(TacoConfigurator.getInstance()
						.getClassToCheck()) && (sigId + "_"
						+ program.getProgramId() + "_0")
							.equals(TacoConfigurator.getInstance()
									.getMethodToCheck()))) {
					continue;
				}
				for (JVariableDeclaration var : program.getParameters()) {
					...
					JField rootField = new JField(var.getVariable(),
							var.getType());
		...
		/**
		 * Used for matching the root nodes so as to only keep those of type
		 * javaType where applicable (the all() method will still return all).
		 * Should be called before accessing any method from this object.
		 */
		...
		/**
		 * Used for matching the root nodes with the QF so as to consider only
		 * those fields in the intersection of both sets. Should be called
		...
		/**
		 * @return All root nodes. Including those of type different than a java
		 *         type.
		 */
		List<JField> all() {
			return rootNodes;
