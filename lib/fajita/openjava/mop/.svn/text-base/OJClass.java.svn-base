...
class OJClassSourceCode extends OJClassImp {
		...
		for (int i = 0, len = mdecls.size(); i < len; ++i) {
			...
			if (mdecl instanceof ClassDeclaration) {
				...
				try {
					...
					DebugOut.println("******************Changing in OJClass.OJClassSourceCode************");
					DebugOut.println("NAME: " + field.getName());
					String cleanName = field.getName().replaceAll("(?si)(/\\*[^\\*]*\\*/|//[^\n]*\n)", "");
					field.setName(cleanName.trim());
					DebugOut.println("NAME: " + field.getName());
					DebugOut.println("******************Changing in OJClass.OJClassSourceCode************");
	...
	OJField addField(OJField field) throws CannotAlterException {
		...
		DebugOut.println("******************Changing in OJClass.addField************");
		DebugOut.println("NAME: " + field.getName());
		String cleanName = field.getName().replaceAll("(?si)(/\\*[^\\*]*\\*/|//[^\n]*\n)", "");
		field.setName(cleanName.trim());
		DebugOut.println("NAME: " + field.getName());
