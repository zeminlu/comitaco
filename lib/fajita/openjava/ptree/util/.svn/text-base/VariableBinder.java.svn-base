...
public class VariableBinder extends ScopeHandler {
	...
	private static void bindName(Environment env, String type, String name) {
		DebugOut.println("******************Changing in VariableBinder.bindName************");
		DebugOut.println("NAME: " + name);
		String cleanName = name.replaceAll("(?si)(/\\*[^\\*]*\\*/|//[^\n]*\n)", "");
		name = cleanName.trim();
		DebugOut.println("NAME: " + name);
		DebugOut.println("TYPE: " + type);
		String cleanType = type.replaceAll("(?si)(/\\*[^\\*]*\\*/|//[^\n]*\n)", "");
		type = cleanType.trim();
		DebugOut.println("TYPE: " + type);
		DebugOut.println("******************Changing in VariableBinder.bindName************");
