...
public class MemberAccessCorrector extends VariableBinder {
	...
	public Expression evaluateDown(FieldAccess ptree)
		...
		DebugOut.println("******************Changing in MemberAccessCorrector.evaluateDown************");
		DebugOut.println("REF: " + ref);
		DebugOut.println("NAME: " + field_name);
		String cleanName = field_name.replaceAll("(?si)(/\\*[^\\*]*\\*/|//[^\n]*\n)", "");
		field_name = cleanName.trim();
		DebugOut.println("NAME: " + field_name);
		DebugOut.println("******************Changing in MemberAccessCorrector.evaluateDown************");
