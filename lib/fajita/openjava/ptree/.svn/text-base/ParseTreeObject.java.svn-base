...
public abstract class ParseTreeObject
	...
	public final void setParent(ParseTreeObject parent) {
		this.parent = parent;
	}
	...
	*/
	...
	public ExpressionStatement getExpressionStatementParent() {
		ParseTreeObject parent = this.getParent();
	...
	public ParseTreeObject getMutationLimitParent() {
		ParseTreeObject parent = this;
		while (parent!=null) {
			if (parent instanceof ExpressionStatement) {
				if ( ((ExpressionStatement) parent).getComment().startsWith("//mutGenLimit") ) return parent;
				else return null;
			}
			if (parent instanceof IfStatement) {
				if ( ((IfStatement) parent).getComment().startsWith("//mutGenLimit") ) return parent;
				else return null;
			}
			if (parent instanceof WhileStatement) {
				if ( ((WhileStatement) parent).getComment().startsWith("//mutGenLimit") ) return parent;
				else return null;
			}
			if (parent instanceof DoWhileStatement) {
				if ( ((DoWhileStatement) parent).getComment().startsWith("//mutGenLimit") ) return parent;
				else return null;
			}
			if (parent instanceof ForStatement) {
				if ( ((ForStatement) parent).getComment().startsWith("//mutGenLimit") ) return parent;
				else return null;
			}
			if (parent instanceof VariableDeclaration) {
				if ( ((VariableDeclaration) parent).getComment().startsWith("//mutGenLimit") ) return parent;
				else return null;
			}
			if (parent instanceof ReturnStatement) {
				if ( ((ReturnStatement) parent).getComment().startsWith("//mutGenLimit") ) return parent;
				else return null;
			}		
